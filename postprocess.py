...
def merge_lmdbs():
    ...
    for i, db_path in enumerate(files):
        ...
        try:
            src_db = lmdb.open(
                db_path, map_size=1e11, readonly=True, readahead=True, lock=False
            )
...
def env_diff(new_env, old_env):
    add = {
        "constants": [
            json.loads(const)
            for const in new_env["constants"]
            if const not in old_env["constants"]
        ],
        "inductives": [
            json.loads(induct)
            for induct in new_env["inductives"]
            if induct not in old_env["inductives"]
        ],
    }
    subtract = {
        "constants": [
            json.loads(const)
            for const in old_env["constants"]
            if const not in new_env["constants"]
        ],
        "inductives": [
            json.loads(induct)
            for induct in old_env["inductives"]
            if induct not in new_env["inductives"]
        ],
    }
    ...
    for i, f in enumerate(files):
        ...
        for proof_name in file_data["proofs"]:
            ...
            for j, const in enumerate(proof_data["env"]["constants"]):
                proof_data["env"]["constants"][j] = json.dumps(
                    proof_data["env"]["constants"][j]
                )
            for j, const in enumerate(proof_data["env"]["inductives"]):
                proof_data["env"]["inductives"][j] = json.dumps(
                    proof_data["env"]["inductives"][j]
                )
            ...
            proofs.append(
                {
                    "name": proof_name,
                    "line_nb": proof_data["line_nb"],
                    "env_delta": env_delta,
                    "steps": proof_data["steps"],
                    "goals": proof_data["goals"],
                    "proof_tree": proof_data["proof_tree"],
                }
            )
