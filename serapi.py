...
def print_mod_path(modpath):
    if modpath[0] == Symbol("MPdot"):
        ...
        return ".".join(
            [symbol2str(x[1]) for x in modpath[1][2][1]][::-1]
    ...
    def send(self, cmd):
        ...
        for item in raw_responses.split("\x00"):
            ...
            if "CoqExn" in item:  # an error occured in Coq
                ...
                raise CoqExn(
                    sexpdata.dumps(parsed_item[2][4]), sexpdata.dumps(parsed_item[2])
                )
    ...
    def query_env(self, current_file):
        ...
        for const in env[1][0][1][0][1]:
            ...
            qualid = (
                ...
                + "."
                + ".".join(
                    [symbol2str(x[1]) for x in const[0][2][1][::-1]]
        ...
        for induct in env[1][0][1][1][1]:
            ...
            qualid = (
                ...
                + "."
                + ".".join(
                    [symbol2str(x[1]) for x in induct[0][2][1][::-1]]
            ...
            inductives.append(
                {
                    "physical_path": physical_path,
                    "blocks": blocks,
                    "is_record": induct[1][0][1][1] != Symbol("NotRecord"),
                    "sexp": sexpdata.dumps(induct),
                }
            )
                        ...
                        hypotheses.append(
                            {
                                ...
                                "term": [
                                    None
                                    if t == []
                                    else self.print_constr(sexpdata.dumps(t))
                                    for t in h[1]
                                ],
