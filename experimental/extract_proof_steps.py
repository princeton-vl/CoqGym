...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--lightmode", type=bool, default=False, help="extracting only a small subset of the proofs")
    args = arg_parser.parse_args()

    print(args)
    print(f"Lightmode: {args.lightmode}  => Only looking at 3 first proof files") if args.lightmode else None

    print("Starting to extract proof steps...\n")
    iter_proofs(
        args.data_root, process_proof, include_synthetic=False, show_progress=True, lightmode=args.lightmode
    )

    print("Starting to make output file...\n")
