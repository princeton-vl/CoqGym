...
def subgoals2hypotheses(script, serapi):
    ...
    if bg_goals != [] or not existing_goal_ids.issubset(
        unsolved_goal_ids.union({current_goal_id})
    ):
...
def set_up_hypotheses(hyps, sexp_cache, serapi):
    ...
    for goal_id, hyp in hyps.items():
        H = "assert (HYPOTHESIS_FROM_SUBGOAL_%d: %s); [> admit | idtac]." % (
            goal_id,
            hyp,
        )
...
def goal2subproof(goal, length, line_nb, script, sexp_cache, serapi):
    ...
    for num_executed, (cmd, cmd_type, _) in enumerate(script, start=line_nb + 1):
        ...
        if args.debug:
            print(
                "SUBPROOF-id%d-len%d %d: %s" % (goal["id"], length, num_executed, cmd)
            )
        serapi.execute(cmd)
    ...
    subproof_data["proof_tree"] = ProofTree(
        subproof_data["steps"], subproof_data["goals"]
    ).to_dict()
    ...
    for num_executed, (cmd, cmd_type, _) in enumerate(script, start=line_nb + 1):
        ...
        if fg_goals != []:
            ...
            for i in range(1, min(args.max_length, len(script) - 2) + 1):
                serapi.push()
                subprf = goal2subproof(
                    g,
                    i,
                    num_executed - 1,
                    script[num_executed - line_nb - 1 : num_executed - line_nb - 1 + i],
                    sexp_cache,
                    serapi,
                )
...
def get_subproofs(human_proof_file, vernac_cmds, sexp_cache, args):
    ...
    with SerAPI(args.timeout, args.debug) as serapi:
        for cmd, _, _ in vernac_cmds[: line_nb + 1]:
            serapi.execute(cmd)
        subproofs_data = record_subproofs(
            line_nb,
            vernac_cmds[line_nb + 1 : line_nb + 1 + len(prf["steps"])],
            sexp_cache,
            serapi,
        )
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(
        description="Extract the synthetic proofs from intermediate goals"
    )
    ...
    arg_parser.add_argument(
        "--timeout", type=int, default=3600, help="Timeout for SerAPI"
    )
    ...
    human_proof_file = (
        dst_filename(args.file, args.data_path) + "-PROOFS/" + args.proof + ".json"
    )
    ...
    subproofs_data = get_subproofs(
        human_proof_file, file_data["vernac_cmds"], sexp_cache, args
    )
